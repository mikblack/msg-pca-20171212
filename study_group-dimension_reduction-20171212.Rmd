...
snpLoc <- snpid2loc(snps,colnames(snpAnsDat))
```
...
```{r}
chrs <- names(table(names(snpLoc)))[c(1,12,16:22,2:11,13:15)]
data.frame(Chr = chrs,
           SNPs = unlist(lapply(chrs, function(x) sum(names(snpLoc)==x))),
           MinPos = unlist(lapply(chrs, function(x) min(snpLoc[which(names(snpLoc)==x)]))),
           MaxPos = unlist(lapply(chrs, function(x) max(snpLoc[which(names(snpLoc)==x)])))) %>% 
...
pcaChr <- list()
for(i in 1:22){
    pcaChr[[i]] = fastPcaGenotypes(snpAnsCount[,names(snpLoc)==paste0("ch",i)], 5)
}
```
